<section data-auto-animate>
    <h3 class="border-b-2 border-orange-300">
        <span>Malveillance 😈</span>
    </h3>
    <span class="fill-green-300">
        Quid de la fiabilité et précision quand on cherche activement à tromper le modèle ?
    </span>
</section>

<section data-auto-animate>
    <h3 class="border-b-2 border-orange-300">
        <span>Malveillance 😈</span>
    </h3>
    <div>
        <div class="flex justify-around items-center">
            <div class="h-36"><img src="images/other_faces/zemmour_old.png"></div>
            <div class="h-36"><img src="images/other_faces/zemmour.jpg"></div>
            <div class="h-36"><img src="images/other_faces/chalamet.jpg"></div>
        </div>
        <div class="fragment m-auto w-1/4"><img src="plots/sim_zem_chal_0.png"></div>
    </div>
</section>

<section data-auto-animate>
    <h3 class="border-b-2 border-orange-300">
        <span>Malveillance 😈</span>
    </h3>
    <div>
        <div class="flex justify-around items-center">
            <div class="h-36 m-12"><img src="images/other_faces/zemmour_old.png"></div>
            <div class="h-36 m-12"><img src="images/other_faces/zemmour.jpg"></div>
            <div class="h-36 m-12"><img src="images/other_faces/chalamet.jpg"></div>
        </div>
    </div>
    <ul class="text-xl">
        <li>L'entraînement consiste à manipuler les paramètres du modèles pour maximiser sa performance</li>
        <li>On peut manipuler la photo pour minimiser sa performance</li>
    </ul>
</section>

<section data-auto-animate>
    <h3 class="border-b-2 border-orange-300">
        <span>Malveillance 😈</span>
    </h3>
    <div>
        <div class="flex justify-around items-center">
            <div class="h-36"><img src="images/other_faces/zemmour_old.png"></div>
            <div class="h-36"><img src="images/other_faces/zemmour.jpg"></div>
            <div class="h-36"><img src="images/other_faces/chalamet.jpg"></div>
        </div>
        <div class="h-64"><img src="plots/100.png"></div>
    </div>
</section>

<style lang="postcss">
    img {
        @apply border-2 border-[#FEE8D5];
        display: block;
        inset: 0;
        width: 100%;
        height: 100%;
        object-fit: contain;
        z-index: -1;
        max-width: 100%;
    }
</style>